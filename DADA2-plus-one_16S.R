# 16S analysis of 'PLUS-ONE' project
# DADA2 pipeline
...
# Raw data should be in folder with _R1.fastq *change as needed
...
write.table(ITS_asv_tax, "ITS_ASVs_taxonomy_JOE.tsv", sep="\t", quote=F, col.names=NA)
...
# WHY IS THE TAXONOMIC ASSIGNMENT DIFFERENT? 
asv_seqs <- taxa_names()

# Random ITS file to check out----
# Originally found in the "SEAGRASS-SEQUENCES" folder so copied and moved to ITS-REDO
ITS_taxa_test <- readRDS("ITS_taxa_mystery.rds")
row.names(ITS_taxa_test) <- sub(">", "", asv_headers)
write.table(ITS_taxa_test, "ITS_ASVs_taxonomy_MYSTERY.tsv", sep="\t", quote=F, col.names=NA)
# Clearly this taxonomic assignment was not correct or something? 
